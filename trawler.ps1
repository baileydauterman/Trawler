...
function Drive-Change {
	...
	if ($drivechange) {
		...
		$dirs = Get-ChildItem $drivetarget -Attributes Directory -Filter "Windows" | Select-Object *
		...
		if (Test-Path "$env_homedrive\Users") {
			...
			$profile_names = Get-ChildItem "$env_homedrive\Users" -Directory
		...
		foreach ($item in Get-TrawlerChildItem -Path $base_key -AsRegistry) {
